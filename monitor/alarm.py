...
SYNCOFF=subprocess.check_output("~/net.ton.dev/scripts/check_node_sync_status.sh | grep error | awk '{print $4}' | tr '[:upper:]' '[:lower:]'",shell=True);
...
if int(SYNC) > int(SYNCalarm1):
  bot.sendMessage(chat_id='-1001416401295', text=str(HOSTNAME)+" SYNC off, SYNC:"+str(SYNC));
if len(SYNCOFF) == 1:
  bot.sendMessage(chat_id='-1001416401295', text=str(HOSTNAME)+" SYNC error");
